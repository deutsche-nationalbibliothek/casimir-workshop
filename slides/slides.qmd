...
::: {style="background: white; color: black; padding: 24px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); min-width: 391px; max-width: 460px; height: 410px;"}
  ...
  * Explore evaluation techniques for evaluating automated subject indexing methods, focussing on practical applications using the **CASIMiR**[^casimir] package.
  * Created as part of DNB-AI-Project, funded by the German Federal Commissioner for Culture and Media (BKM) [^bkm]
...
publication_data <- read_csv("dnb_publication_volumes.csv")  |>
  pivot_longer(cols = -year, names_to = "Type", values_to = "Count")

manual_fill_scale <- scale_fill_manual(
  labels = c("ebooks" = "Online Monografies",
    "print" = "Print Monografies",
    "articles" = "Online Articles"),
  values = c("ebooks" = "#E41A1C",
    "print" = "#377EB8",
    "articles" = "#4DAF4A")
)

ggplot(publication_data, aes(x = year, y = Count, fill = Type)) + 
  geom_bar(stat = "identity", position = "stack") +
  manual_fill_scale + 
  scale_y_continuous(labels = scales::comma, limits = c(0, 3.2e6)) + 
  xlab("Publication Year") +  ylab("Number of Publications") +
  ggtitle("Publication volumes collected at the German National Library")
...
* statistical approaches don't need prefLabels: they do not interpret the description of labels
...
Going for generative LLM-powered methods:
  ...
  * use few-shot prompting to guide LLMs towards desired behaviours
...
::: {.column width="80%"}
{{< include figures/example-table.qmd >}}
:::
